namespace Lynkly.Shared.Kernel.Core.Helpers.Threading;
...
public static class TaskHelper
{
    ...
    public static async Task<T> WithTimeoutAsync<T>(Task<T> task, TimeSpan timeout, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw SharedKernelException.InvalidArgument("Argument 'timeout' must be greater than zero.");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw SharedKernelException.InvalidArgument("Argument 'maxDegreeOfParallelism' must be greater than zero.");
        }
        ...
        var workerCount = System.Math.Min(maxDegreeOfParallelism, factories.Count);
        var nextIndex = -1;
